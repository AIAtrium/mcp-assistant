    ...
    try:
        ...
        while True:
            ...
            if int(time.time()) % 1 == 0:
                print(
                    f"Waiting for process... {int(time.time() - connection_time)}s elapsed"
                )
        ...
        try:
            with open(token_file, "r") as f:
                ...
                if "access_token" in token_data:
                    ...
                    print(
                        f"Using access token: {access_token[:5]}...{access_token[-5:]}"
                    )
                    break
        ...
        for fd, event in ready:
            if fd == process.stderr.fileno():
                ...
                if line:
                    print(f"STDERR: {line.strip()}")
                    if (
                        "Connected to remote server" in line
                        or "Proxy established successfully" in line
                    ):
