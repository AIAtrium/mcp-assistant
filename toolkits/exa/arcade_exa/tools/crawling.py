...
async def crawling(
    ...
    url: Annotated[
        str,
        "The URL to crawl (e.g., 'exa.ai')"
    ],
) -> Annotated[
    dict,
    "A dictionary containing the crawled content from the specified URL"
]:
    """
    ...
    async with httpx.AsyncClient(timeout=25) as client:
        ...
        data = response.json()
        if not data or (isinstance(data, dict) and (not data.get("results") or len(data.get("results", [])) == 0)):
            return {
                "content": [{
                    "type": "text",
